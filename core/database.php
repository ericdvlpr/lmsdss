...
error_reporting(0);
 ...
 {
   // BUG: : Check report
   // BUG: : Check check in/ out book
   // BUG: : Check reserve book
   // BUG: : Reserve book modal
      //crud class
      public $connect;
    ...
    //faculty index
    public function get_bookBorrow_faculty($query){
               $output = '';
                ...
                if($row->activity=='overdue'){
                 ...
                 $output .= '
                   <div class="callout '.$status.' ">
                      ...
                      <p> Date Borrowed: '.$row->on_date.'</p>
                      <p> Date Due: '.$row->due_date.'</p>
                    </div>
                 ';
               }
               return $output;
    }
    public function get_bookRefer_faculty($query){
               $output = '';
               $result = $this->execute_query($query);
               while($row = mysqli_fetch_object($result))
               {
                if($row->status==0){
                 ...
                 $output .= '
                   <div class="callout '.$status.' ">
                      <h4>'.$row->book_title.'<br /><small>'.$row->author.'</small></h4>
                      <p> Date Requested: '.$row->date_requested.'</p>
    ...
    public function get_bookRequest_faculty($query){
      $output = '';
           $result = $this->execute_query($query);
           $output .= '';
           while($row = mysqli_fetch_object($result))
    ...
    //REPORTS TABLES
     public function get_bookReport($query) {
          ...
          public function get_periodicalReport($query) {
               $output = '';
               $result = $this->execute_query($query);
               while($row = mysqli_fetch_object($result))
               {
                ...
                $date=date_create($row->date_added);
                    ...
                    <tr>
                         <td>'.$row->per_num.'</td>
                         <td>'.$row->per_article.'</td>
                         <td>'.$row->title_periodical.'</td>
                         <td>'.$row->publisher.'</td>
                         <td>'.$row->volumn_num.'</td>
                         <td>'.$row->location.'</td>
                    </tr>
                    ';
               }
          ...
          public function get_audioReport($query) {
               $output = '';
               $result = $this->execute_query($query);
               while($row = mysqli_fetch_object($result))
               {
           ...
           while($row = mysqli_fetch_object($result))
           {
            $count = $this->count_borrowed_books_student($row->bbid);
               $output .= '
                <tr>
                     <td>'.$row->bbid.'</td>
       ...
       public function count_borrowed_books_student($id){
          $result_1 =$this->execute_query("SELECT COUNT('brdt_id') FROM borrow_details WHERE bbid='".$id."'");
           $count = mysqli_num_rows($result_1);
           return $count;
        }
...
//---------------
      ...
      public function get_periodical_data($query) {
           $output = '';
           $result = $this->execute_query($query);
            ...
            if($row->copies==0){
              ...
              $statusColor = 'class="warning"';
                ...
                <tr>
                     ...
                     <td>'.$row->issue_num.'</td>
                     <td>'.$row->location.'</td>
                     <td><button type="button" name="view" id="'.$row->periodical_id.'" class="btn btn-primary btn-xs viewPeriodical">View</button><button type="button" name="update" id="'.$row->periodical_id .'" class="btn btn-success btn-xs updatePeriodical">Update</button></td>
                </tr>
                ';
           }
      ...
      public function get_audio_data($query) {
           $output = '';
           $result = $this->execute_query($query);
            ...
            if($row->copies==0){
              ...
              $statusColor = 'class="warning"';
                ...
                <tr>
                     ...
                     <td>'.$row->author.'</td>
                     <td>'.$row->av_title.'</td>
                     <td align="right">'.$row->copies.'</td>
                     <td><button type="button" name="view" id="'.$row->av_id.'" class="btn btn-primary btn-xs viewAudio">View</button><button type="button" name="update" id="'.$row->av_id .'" class="btn btn-success btn-xs updateAudio">Update</button></td>
                </tr>
                ';
           }
      ...
      public function get_author_data($query) {
         ...
         public function get_book_issued_data() {
           ...
           $query = "SELECT s.student_name AS Name, f.faculty_name AS fname, bd.borrow_no AS borrow_no, bd.activity AS status FROM borrow_details bd LEFT JOIN students s ON s.student_id = bd.bbid LEFT JOIN faculty f ON f.faculty_no = bd.bbid";
   ...
   public function get_selected_data($id,$mid)
      {
         ...
         $query4 = "SELECT * FROM borrow_book bb LEFT JOIN borrow_details bd ON bd.borrow_no = bb.borrow_no WHERE bb.book_no = '".$id."' AND bd.bbid = '".$mid."' AND (bd.activity = 'reserved' OR bd.activity = 'borrowed' OR bd.activity = 'overdue')";

         $query5 = "SELECT bb.copies AS copy FROM borrow_book bb LEFT JOIN borrow_details bd ON bb.borrow_no = bd.borrow_no WHERE bd.bbid = '".$mid."'";
        ...
        ';
         ...
         if($calc == 0){
            ...
            if(mysqli_num_rows($result4)){
              ...
              $output .= 'Would you like to reserve this book? type Yes, or No.|true|'.$title.'/'.$row->ab_author.'|'.$calc.'|N/A';
            }
         }
         //*/
         return $output;
            ...
            while($row = mysqli_fetch_object($result))
            {
               $output .= '
                <tr>
                  <td>';
                  if($row->ab_type == 2){
                    $output .= '<div class="book_title">'.$row->ab_title.' (Audio Visuasl)</div>';
                  }else if($row->ab_type == 3){
 ...
 public function tapin_data($query,$id){
     ...
     public function get_issue_data($id){
          ...
          public function get_return_data($IDs,$Contact){
              ...
              $penque = "SELECT men_2 FROM maintenace";
              $penres = $this->execute_query($penque);
              $penrow = mysqli_fetch_object($penres);
              $result = $this->execute_query($query);
              $result2 = $this->execute_query($query2);

              if(mysqli_num_rows($result) == mysqli_num_rows($result2)){

                  $olquery = "SELECT * FROM borrow_details WHERE borrow_no='".$IDs."' AND (activity = 'overdue' OR activity = 'limbo')";
                  $olresult = $this->execute_query($olquery);
                  if(mysqli_num_rows($olresult)){
                    $dcal = 0;
                    $dcalT = 0;

                    $output = "Warning: The Following Books are overdue: <ul>";

                    $osbque = "SELECT bk.book_title AS title1, av.av_title AS title2, p.per_article AS title3, bb.due_date AS dues FROM borrow_book bb LEFT JOIN book bk ON bk.book_no = bb.book_no LEFT JOIN audio_visual av ON av.av_num = bb.book_no LEFT JOIN periodical p ON p.per_num = bb.book_no WHERE bb.borrow_no = '".$IDs."'";
                    $osbres = $this->execute_query($osbque);
                    while ($osbrow = mysqli_fetch_object($osbres)){
                      if(date('Y-m-d')>=$osbrow->dues){

                        $dcal = round((time() - strtotime($osbrow->dues))/86400) * $penrow->men_2;

                        $dcalT = $dcalT + $dcal;

                        if($osbrow->title1 == NULL && $osbrow->title3 == NULL){
                          $output .= "<li>".$osbrow->title2." (Audio Visual) (".$osbrow->dues.") (Penalty: P".$dcal.")</li>";
                        }else if($osbrow->title1 == NULL && $osbrow->title2 == NULL){
                          $output .= "<li>".$osbrow->title3." (Periodical) (".$osbrow->dues.") (Penalty: P".$dcal.")</li>";
                        }else{
                          $output .= "<li>".$osbrow->title1." (".$osbrow->dues.")  (Penalty: P".$dcal.")</li>";
                        }

                      }
                    }
                      $output .= "<ul> Total Penalty: P".$dcalT;
                    ...
                    $query3 = "SELECT * FROM over_due WHERE issue_id='".$IDs."'";
                    $result3 = $this->execute_query($query3);

                    if(mysqli_fetch_assoc($result3)){
                      $delque = "DELETE FROM over_due WHERE issue_id = '".$IDs."'";
...
//check status
           public function time_due_check(){
            ...
            $query = "SELECT bd.borrow_no AS borrow_no, bd.bbid AS bbid, s.contact AS contact FROM borrow_details bd LEFT JOIN students s ON bd.bbid = s.student_id WHERE bd.activity = 'borrowed' OR bd.activity = 'limbo'";
            $result = $this->execute_query($query);
            $output = '';
            $date = date('Y-m-d');
            ...
            while($row=mysqli_fetch_object($result)){// Start
                ...
                if($ovli){
                    ...
                    if(mysqli_num_rows($odres) == 0){
                        $odaqu = "INSERT INTO `over_due`(issue_id, prev_send, next_send, bbid) VALUES ('".$row->borrow_no."', '".$date."', '".$dues."', '".$row->bbid."')";
            ...
            // Message Division
               if($newbie){
                  $output .= "][\t You have Books Overdue: \n";//$this->get_message_head('ODBRBK002')." \n";
                  ...
                  while($obrow = mysqli_fetch_object($obret)){
                      if(date('Y-m-d')>=$obrow->due){
                        ...
                        $dcal = round((time() - strtotime($obrow->due))/86400) * $penrow->men_2;

                        $dcalT = $dcalT + $dcal;

                        // if($obrow->title1 == NULL && $obrow->title3 == NULL){
                        //   $output .= "\t\t".$obrow->title2." (Audio Visual) (".$obrow->due.") (Penalty: P".$dcal.") \n";
                        // }else if($obrow->title1 == NULL && $obrow->title2 == NULL){
                        //   $output .= "\t\t".$obrow->title3." (Periodical) (".$obrow->due.") (Penalty: P".$dcal.")\n";
                        // }else{
                        //   $output .= "\t\t".$obrow->title1." (".$obrow->due.")  (Penalty: P".$dcal.")\n";
                        // }

                      }
                  }
                  $output .= " \t Total Penalty: ".$dcalT."|".$row->contact;
                  //$output .= $this->get_message_foot('ODBRBK002')."|".$row->contact;
                  $empty = false;
                }
              //Message V2
                $odcqu = "SELECT s.contact AS contact, od.issue_id AS borrow_no, od.prev_send AS prv, od.next_send AS nxt, od.sent AS sent FROM over_due od LEFT JOIN students s ON s.student_id = od.bbid ";
                $odcre = $this->execute_query($odcqu);
                while($odcrow = mysqli_fetch_object($odcre)){

                  if(((date('Y-m-d')) == $odcrow->nxt) && ($odcrow->sent == '0')){

                    $output .= "][\t Warning Overdue: \n";//.$this->get_message_head('ODBRBK002')."\n";
                    ...
                    $output .= "\t Total Penalties: ".$dcalT."|".$odcrow->contact."";//$this->get_message_foot('ODBRBK002')."|".$odcrow->contact."";
         ...
         function time_ago($timestamp) {
            ...
            public function get_student_profile($query) {
           $output = '';
           $result = $this->execute_query($query);
           while($row = mysqli_fetch_object($result))
           {
                        ...
                        <div class="box box-widget widget-user-2">
                              <div class="widget-user-header bg-blue">
                                <div class="widget-user-image">
                                  <img src="images/user.jpg" width="300" height="300" class="img-circle" alt="User Image">
                                </div>
                                <!-- /.widget-user-image -->
                                <h1 class="widget-user-username">'.$row->student_name.'</h1>
                                <h3 class="widget-user-desc">'.$row->student_id.'</h3>
                              </div>
                              <div class="box-footer no-padding">
                               <ul class="nav nav-stacked">
                                  <li><a href="#" disabled>Department:<span class="pull-right badge bg-blue">'.$row->department_name.'</span></a></li>
                                  <li><a href="#" disabled>Course:<span class="pull-right badge bg-blue">'.$row->course_code.'</span></a></li>
    ...
    public function get_faculty_profile($query) {
           $output = '';
           $result = $this->execute_query($query);
           while($row = mysqli_fetch_object($result))
           {
                  ...
                  <div class="col-md-4">
                        <div class="box box-widget widget-user-2">
                              <div class="widget-user-header bg-red">
                                ...
                                <!-- /.widget-user-image -->
                                <h1 class="widget-user-username">'.$row->faculty_name.'</h1>
                                <h3 class="widget-user-desc">'.$row->faculty_no.'</h3>
                              </div>
                              <div class="box-footer no-padding">
                               <ul class="nav nav-stacked">
                                  <li><a href="#" disabled>Department:<span class="pull-right badge bg-red">'.$row->department_name.'</span></a></li>
      ...
      public function get_profile($query) {
           $output = '';
           $result = $this->execute_query($query);
           while($row = mysqli_fetch_object($result))
           {
                  ...
                  <div class="col-md-4">
                        <div class="box box-widget widget-user-2">
                              <div class="widget-user-header bg-yellow">
                                ...
                                <!-- /.widget-user-image -->
                                <h1 class="widget-user-username">'.$row->username.'</h1>
                                <h3 class="widget-user-desc">'.$department.'</h3>
                              </div>
                              <div class="box-footer no-padding">
                               <ul class="nav nav-stacked">
                                  <li><a href="#" disabled>Access:<span class="pull-right badge bg-blue">'.$access.'</span></a></li>
      ...
      public function check_reserve_data(){
           //-----------------------------------------------------------------------------------------------------------------
            //Reserve Limit Exeed
           $rsbdquery = "SELECT bd.borrow_no AS borrow_no From borrow_details bd WHERE bd.activity = 'reserved'";
           $rsbdresult = $this->execute_query($rsbdquery);
            while($rsbdrow = mysqli_fetch_object($rsbdresult)){
                $rsbbquery = "SELECT bb.due_date AS Dues, bb.id AS IDs FROM borrow_book bb WHERE bb.borrow_no = '".$rsbdrow->borrow_no."'";
                $rsbbresult = $this->execute_query($rsbbquery);
                while ($rsbbrow = mysqli_fetch_object($rsbbresult)) {
                    if(date('Y-m-d') >= $rsbbrow->Dues){
                       $rsbbdelque = "DELETE FROM borrow_book WHERE id = '". $rsbbrow->IDs."'";
                       $rsbbdelres = $this->execute_query($rsbbdelque);
                    }
                }
            }

           $rsbdquery2 = "SELECT bd.borrow_no AS borrow_no From borrow_details bd WHERE bd.activity = 'reserved'";
           $rsbdresult2 = $this->execute_query($rsbdquery2);

            while($rsbdrow2 = mysqli_fetch_object($rsbdresult2)){
                $rsbbquery2 = "SELECT bb.due_date AS Dues, bb.id AS IDs FROM borrow_book bb WHERE bb.borrow_no = '".$rsbdrow2->borrow_no."'";
                $rsbbresult2 = $this->execute_query($rsbbquery2);

                if(!mysqli_num_rows($rsbbresult2)){
                  $rsbddelque2 = "DELETE FROM borrow_details WHERE borrow_no = '". $rsbdrow2->borrow_no."'";
                  $rsbddelres2 = $this->execute_query($rsbddelque2);
                }
            }
